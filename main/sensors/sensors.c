...
char* measurements_to_json(const mesurements_t *m)
{
    ...
    cJSON_AddStringToObject(root, "voltage1", buf);

    snprintf(buf, sizeof(buf), "%.2f", m->voltage2);
    cJSON_AddStringToObject(root, "voltage2", buf);

    snprintf(buf, sizeof(buf), "%.2f", m->current1);
    cJSON_AddStringToObject(root, "current1", buf);

    snprintf(buf, sizeof(buf), "%.2f", m->current2);
    cJSON_AddStringToObject(root, "current2", buf);

    snprintf(buf, sizeof(buf), "%.2f", m->velocity);
    cJSON_AddStringToObject(root, "velocity", buf);
